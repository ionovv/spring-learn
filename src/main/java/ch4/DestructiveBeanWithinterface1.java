...
public class DestructiveBeanWithinterface1 {
    private File file;
        ...
        if (filePath == null) {
            throw new IllegalArgumentException(
                    "You must specify the filePath property of "// Свойство filePath должно быть установлено в классе
                            + DestructiveBeanWithinterface1.class);
        }
    ...
    public static void main(String[] args) throws Exception {
        ...
        ctx.load("classpath:config/ch4/app-context-xml-DestructiveBeanWithinterface1.xml");
        ctx.registerShutdownHook();
        ctx.refresh();

        DestructiveBeanWithinterface1 bean = (DestructiveBeanWithinterface1) ctx.getBean("destructiveBean");
    }
}